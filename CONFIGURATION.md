...
## FireStick 4K Optimization (New in v2.0)

The scoring algorithm now includes FireStick 4K specific optimizations that prioritize:
- **Optimal bitrate** over maximum bitrate (faster channel switching)
- **Fast startup** for better user experience
- **Reliability** with stricter error penalties
- **Efficient codecs** (HEVC preference)

### Enabling FireStick Optimization

In `config.yaml`:
```yaml
scoring:
  use_firestick_optimization: true  # Default: true
```

### Comparing Old vs New Scoring

To see the difference:
1. Run quality check with `use_firestick_optimization: true`
2. Check the CSV output - score components are included
3. Top streams will prioritize:
   - 1080p HEVC at optimal bitrate over 4K at excessive bitrate
   - Streams with <1 second startup over slower high-bitrate streams
   - Zero-error streams over streams with any errors
...
```yaml
scoring:
  use_firestick_optimization: false
```
...
```yaml
scoring:
  use_firestick_optimization: true

ordering:
  resilience_mode: true
  similar_score_delta: 10
```
