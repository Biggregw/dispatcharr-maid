...
def _poll_quality_insights():
    ...
    try:
        ...
        with _quality_insight_lock:
            if _quality_insight_api is api:
                _quality_insight_api = None
        logging.warning("Quality insight poll failed: %s", exc)
        return False
...
def _compute_quality_insight_status(window_hours=12):
    ...
    for item in insights:
        rag = item.get("rag_status", "green")
        if status_order.get(rag, 0) > status_order.get(worst, 0):
            worst = rag
    return worst
...
def index():
    ...
    last_issue_summary = _get_quality_insight_last_issue_summary()
